...
class VolunteerMyTasksScreen extends StatefulWidget {
  ...
  @override
  State<VolunteerMyTasksScreen> createState() =>
      _VolunteerMyTasksScreenState();
}

class _VolunteerMyTasksScreenState
    extends State<VolunteerMyTasksScreen> {
  int _filterIndex = 0;
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              data.updateTask(task.copyWith(
                ...
                photoUrl:
                    'https://placeholder.com/photo_${task.id}.jpg',
              ));
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    final submitted = userTasks
        .where((t) => t.status == TaskStatus.submitted)
        .length;
    final approved =
        userTasks.where((t) => t.status == TaskStatus.approved).length;
